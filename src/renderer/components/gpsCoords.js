...
mapboxgl.accessToken =
  'pk.eyJ1IjoiZGFuaWVsaG9uaWVzIiwiYSI6ImNrOGVvb2FiYzAzNGszbXRwdDEyNDlpcXoifQ.O-1QpnP-e9XEqRbCruhUwA';
...
function GpsCoords() {
  ...
  const handleCloseConfirm = () => {
    console.log(goToPos);
    fetch(
      'http://localhost:8081/goto?longitude_deg=' +
        goToPos.lngLat.lng.toFixed(6) +
        '&latitude_deg=' +
        goToPos.lngLat.lat.toFixed(6) +
        '&altitude_m=20&yaw_deg=0',
      {
        method: 'GET',
        headers: {
  ...
  useEffect(() => {
    if (map.current) {
      ...
      missionItems.map((item) => {
        if (item.command == 16) {
          const marker = new mapboxgl.Marker({
            ...
            .setLngLat([(item.y * 1) / 10000000, (item.x * 1) / 10000000])
            .addTo(map.current);
          marker.getElement().innerHTML =
            '<div class="missionMarker">' + item.seq + '</div>';
      ...
      const markerCoords = missionItems
        .filter((item) => item.command == 16 || item.command == 22)
        .map((item) => [(item.y * 1) / 10000000, (item.x * 1) / 10000000]);
  ...
  useEffect(() => {
    ...
    mark.current = new mapboxgl.Marker({ anchor: 'center', offset: [0, 0] })
      .setLngLat([gpsPos.longitude_deg, gpsPos.latitude_deg])
      .addTo(map.current);
    // set marker icon to font awesome icon
    mark.current.getElement().innerHTML =
      '<div class="drone"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--! Font Awesome Pro 6.4.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path d="M429.6 92.1c4.9-11.9 2.1-25.6-7-34.7s-22.8-11.9-34.7-7l-352 144c-14.2 5.8-22.2 20.8-19.3 35.8s16.1 25.8 31.4 25.8H224V432c0 15.3 10.8 28.4 25.8 31.4s30-5.1 35.8-19.3l144-352z"/></svg></div>';
  ...
  return (
    <div>
      ...
      <PrettyText id="coordinates">
        {gpsPos.latitude_deg.toFixed(4)}, {gpsPos.longitude_deg.toFixed(4)},{' '}
        {gpsPos.absolute_altitude_m.toFixed(2)}
      </PrettyText>
      <Modal show={showGoto} onHide={handleClose}>
        ...
        <Modal.Body>
          Confirm GoTo. Drone will move to Point you clicked on!
        </Modal.Body>
        <Modal.Footer>
