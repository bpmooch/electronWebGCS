...
function MissionButton() {
  ...
  useEffect(() => {
    const timer = setInterval(async () => {
      ...
      setMissionRunning(
        newMissionRunning.flight_mode == 'FLIGHT_MODE_MISSION' ? true : false
      );
    }, 100);
  ...
  return (
    <>
      {' '}
      <Button variant="dark" onClick={handleShow} hidden={missionRunning}>
        <FontAwesomeIcon icon={faDrawPolygon} color="white" />
      </Button>
      <Button variant="dark" onClick={handlePause} hidden={!missionRunning}>
