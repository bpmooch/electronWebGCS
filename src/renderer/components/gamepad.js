...
export default function Gamepad() {
  ...
  useEffect(() => {
    ...
    if ('axes' in defaultGamepad) {
      ...
      setCommands([
        defaultGamepad.axes[0],
        (defaultGamepad.axes[1] * -1 + 1) / 2,
        defaultGamepad.axes[2],
        defaultGamepad.axes[3] * -1,
      ]);
      console.log(commands);
      if (mcactive) {
        fetch(
          'http://localhost:8081/setManualControl?x=' +
            commands[3].toFixed(2) +
            '&y=' +
            commands[2].toFixed(2) +
            '&z=' +
            commands[1].toFixed(2) +
            '&r=' +
            commands[0].toFixed(2),
          {
            method: 'GET',
            headers: {
              Accept: 'application/json',
  ...
  return (
    <Button
      variant={mcactive ? 'success' : 'dark'}
      onClick={() => {
        StartPositionControl();
        setMCActive(!mcactive);
      }}
    >
